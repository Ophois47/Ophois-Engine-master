...
public:
	...
	ophoisSprite(int w, int h)
	{
		Create(w, h);
	}

	ophoisSprite(std::wstring sFile)
	{
		if (!Load(sFile))
			Create(8, 8);
	}
...
class ophoisConsoleGameEngine
{
public:
	ophoisConsoleGameEngine()
	{
		m_nScreenWidth = 80;
		m_nScreenHeight = 30;
	...
	void DrawSprite(int x, int y, ophoisSprite *sprite)
	{
		if (sprite == nullptr)
			return;
	...
	void DrawPartialSprite(int x, int y, ophoisSprite *sprite, int ox, int oy, int w, int h)
	{
		if (sprite == nullptr)
			return;
